<template>
  <div class>
    ...
    <div class="gallery-indicator">
      <div
        class="indicator bg-yellow-lighter"
      >{{`${selected.item + 1}/${componentData.answers.length}`}}</div>
    </div>
    ...
    <no-evaluable-buttons
      :retry="retry"
      :verify="validateAnswers"
      :disable-verify="disable_validate"
    ></no-evaluable-buttons>
    ...
    <div class="eva-modal default left-0" :class="{'show': modals.show}">
      <div class="background"></div>
...
export default {
  ...
  components: {
    "no-evaluable-buttons": () => import("./NoEvaluableButtons"),
  ...
  computed: {
    ...
    items() {
      return this.componentData.answers;
    },
    prev_item() {
  ...
  methods: {
    retry() {
      for (let i = _.size(this.componentData.answers); i > 0; i--) {
        let item_configurations = this.componentData.answers[i - 1]
          .configurations;
        for (let j = _.size(item_configurations.selected_options); j > 0; j--) {
          item_configurations.selected_options.splice(j - 1, 1);
        }
      }
      this.selectItem(0);
      this.disable_validate = false;
    },
    async validateAnswers() {
      let sort_success = await this.checkAnswer();
      if (sort_success) {
        this.sort_success = sort_success;
        this.disable_validate = true;
      }
      this.modals.show = true;
    },
    checkAnswer(event) {
      let correct = false;
      for (let i = 0; i < _.size(this.componentData.answers); i++) {
        let item_configurations = this.componentData.answers[i].configurations;
    ...
    getUserAnswers() {
      ...
      _.each(this.items, answer => {
        let tmp = {
          ...
          selected_options: answer.configurations.selected_options
        };
...
.gallery-indicator {
  @apply flex justify-center mb-12 mt-12;
...
.gallery-content {
  @apply flex mb-4 items-center justify-center text-5xl;
  color: #2d7efc;
