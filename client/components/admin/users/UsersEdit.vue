<template>
  <div class="container block-centered ml-md-5 ml-lg-0">
    <form class="card">
      <div class="card-content control">
        <div class="row">
          ...
          <div class="col-12 row margin-b-10 bt-4">
            <div class="col-12 col-md-4">
              <b-field
                  custom-class="has-text-weight-bold font-18"
        ...
        <br>
        <div class="row justify-content-center pt-5" v-if="is_editing">
          <div class="col-12 col-md-3">
            <button type="button" class="button is-link create-user-btn width-100" @click="validateForm" :disabled="errors.any()">
              {{$t('messages.update')}}
            </button>
          </div>
          <div class="col-12 col-md-3">
            <button type="button" class="button is-link create-user-btn width-100" @click="is_editing = false">
              {{$t('messages.cancel')}}
            </button>
          </div>
        </div>
        <div class="row justify-content-center" v-else>
          <div class="col-12 col-md-3">
            <button type="button" class="button is-link create-user-btn width-100" @click="is_editing = true">
              {{$t('messages.edit')}}
            </button>
          </div>
        </div>
...
export default {
  ...
  computed: {
    disabled () {
      return !this.is_editing || this.user.owner
    }
  },
  methods: {
    setFilters () {
    ...
    setForm () {
      ...
      this.form = form;
      this.form.role = _.get(form, ['role', 'id']);
      this.form.birthday = new Date(_.get(form, ['birthday']));
      this.form.company_id = _.get(form, ['company', 'id']);
      delete this.form.company;
      this.form.taxonomy_areas = this.user.taxonomy_areas.map(item => {
        return item.id;
      });
      this.form.taxonomy_positions = this.user.taxonomy_positions.map(item => {
        return item.id;
      });
      this.form.taxonomy_abilities = this.user.taxonomy_abilities.map(item => {
        return item.id;
      });
      this.form.taxonomy_teams = this.user.taxonomy_teams.map(item => {
        return item.id;
      });
      this.form.taxonomy_certifications = this.user.taxonomy_certifications.map(item => {
        return item.id;
      });
      this.form.taxonomy_customs = this.user.taxonomy_customs.map(item => {
        return item.id;
      });
    },
    clearFilters () {
    ...
    patch () {
      let title = this.$t('messages.saving');
      let message = this.$t('messages.sending_data');
      this.$snotify.async(title, message, () => new Promise((resolve, reject) => {
        ...
        return axios.patch(route('users.update', this.form.id), this.form)
            .then(response => {
