...
<script>
    export default {
        name: 'apithy-experience-adventure-list',
        props: {
            experience:{},
            title: {
                type: String,
        ...
        methods: {
            ...
            changeSession(session, key) {
                console.log(session);
                if (
                    session.current_enrollment_progress &&
                    typeof session.current_enrollment_progress.id !== 'undefined' &&
                    session.current_enrollment_progress.status != this.apithy_constants.SESSION_STATUS_BLOCKED
                ) {
                    ...
                    if(this.experience.is_auth_user_purchased){
